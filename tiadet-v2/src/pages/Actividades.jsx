...
const TIPO_CFG = {
  ...
  foro:     { icon:'💬', label:'Foro' },
  examen:   { icon:'📝', label:'Examen' },
}
const ESTADO_CFG = {
  completada: { badge:'badge-green',  label:'Completada' },
  pendiente:  { badge:'badge-warn',   label:'Pendiente' },
  proximo:    { badge:'badge-gray',   label:'Próximo' },
}
const FILTROS = ['Todas', 'Pendiente', 'Completada', 'Próximo']
  ...
  return (
    <Layout title="Actividades">
      ...
      <div className="filtros-row">
        {FILTROS.map(f=>(
          <button key={f} className={`filtro-btn ${filtro===f?'filtro-active':''}`} onClick={()=>setFiltro(f)}>{f}</button>
        ))}
      </div>
              ...
              <div className="act-head" onClick={()=>setAbierta(open?null:a.id)}>
                ...
                <div className="act-info">
                  ...
                  <div className="act-meta">
                    ...
                    {mod && <span className="act-mod-tag">Módulo {mod.numero}</span>}
                ...
                <div className="act-body">
                  ...
                  {a.tipo==='tarea' && a.estado==='pendiente' && (
                    <div className="entrega-blk">
                      ...
                      <label className="form-label" style={{marginBottom:6}}>Link (Google Drive, YouTube, etc.)</label>
                      <input type="url" placeholder="https://..." value={linkEntrega[a.id]||''} onChange={e=>setLinkEntrega(p=>({...p,[a.id]:e.target.value}))} />
                      <label className="form-label" style={{margin:'12px 0 6px'}}>O subir archivo</label>
                      <label className="file-upload-area">
                        <input type="file" style={{display:'none'}} onChange={e=>handleArchivo(a.id,e)} />
                        {archivoNombre[a.id] ? <span className="file-chosen">📎 {archivoNombre[a.id]}</span> : <span className="file-placeholder">Seleccioná un archivo (PDF, DOC…)</span>}
                      </label>
                  ...
                  {a.estado==='proximo' && <p className="act-proximo-msg">Esta actividad aún no está disponible.</p>}
