...
export default function TFI() {
  ...
  return (
    <Layout title="Trabajo Final Integrador">
          ...
          <div className="card tfi-enunciado-card">
            <div className="tfi-header">
              ...
              <div className="tfi-fecha-badge">
                <span className="badge badge-warn">📅 Entrega: {tfi.fecha_entrega}</span>
              </div>
            </div>
            ...
            <h2 className="section-label">Criterios de evaluación</h2>
            <div className="tfi-criterios">
              {tfi.criterios.map((c,i)=>(
                <div key={i} className="tfi-criterio-row">
                  ...
                  <span className="badge badge-blue">{c.peso}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="card">
            ...
            {enviado ? (
              <div className="tfi-enviado">
                <span className="tfi-enviado-icon">✓</span>
                <p className="tfi-enviado-txt">Tu TFI fue enviado correctamente.</p>
                <p style={{fontSize:12,color:'var(--text-3)',marginTop:6}}>La docente recibirá una notificación.</p>
              ...
              <>
                <label className="form-label" style={{marginBottom:6}}>Link (Google Drive, GitHub, etc.)</label>
                <input type="url" placeholder="https://drive.google.com/..." value={link} onChange={e=>setLink(e.target.value)} />

                <label className="form-label" style={{margin:'14px 0 6px'}}>Subir archivo (PDF, ZIP…)</label>
                <label className="file-upload-area">
                  <input type="file" style={{display:'none'}} accept=".pdf,.doc,.docx,.zip" onChange={e=>setArchivo(e.target.files[0])} />
                  {archivo
                    ? <span className="file-chosen">📎 {archivo.name}</span>
                    : <span className="file-placeholder">Seleccioná un archivo</span>
                  }
                </label>
                ...
                {!link && !archivo && (
                  <p style={{fontSize:12,color:'var(--text-3)',marginTop:8,textAlign:'center'}}>
                    Adjuntá un link o un archivo para enviar.
                  </p>
                )}
              </>
            )}
