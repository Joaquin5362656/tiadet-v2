...
export function AuthProvider({ children }) {
  ...
  function login(dni, password) {
    ...
    if (u) { setUsuario(u); localStorage.setItem(LS_KEY_SESSION, JSON.stringify(u)); return true }
    return false
  }
  ...
  function register({ nombre, dni, email, password }) {
    if (usuarios.find(u => u.dni === dni.trim())) return { ok: false, error: 'El DNI ya está registrado.' }
    if (usuarios.find(u => u.email === email.trim())) return { ok: false, error: 'El email ya está registrado.' }
    const nuevo = {
      id: Date.now(), dni: dni.trim(), email: email.trim(), password,
      nombre: nombre.trim(), rol: 'alumno', foto: null,
      legajo: `DOC-${String(usuarios.length + 1).padStart(3, '0')}`,
    }
    const lista = [...usuarios, nuevo]
    setUsuarios(lista); guardarUsuarios(lista)
    setUsuario(nuevo); localStorage.setItem(LS_KEY_SESSION, JSON.stringify(nuevo))
    return { ok: true }
  }

  function logout() {
    setUsuario(null); localStorage.removeItem(LS_KEY_SESSION)
  }
  ...
  return (
    <AuthContext.Provider value={{ usuario, login, loginInvitado, register, logout, actualizarFoto, usuarios: alumnosFiltrados }}>
      {children}
